...
document.addEventListener('DOMContentLoaded', function () {
	...
	var ID_FILE_PICKER        = '_bimeson_setting_pubfile_multi';
	...
	var SEL_FILTER_BUTTON     = '.bimeson_admin_filter';
	var SEL_LOADING_SPIN      = '.bimeson_admin_filter_loading';
	var SEL_RESULT_HOLDER     = '#_bimeson_pub_items';
	var SEL_FIRST_KEY_OMITTED = '#_bimeson_pub_first_key_omitted';

	var KEY_BODY       = 'body';
	var KEY_ENGLISH    = 'english';
	var KEY_JAPANESE   = 'japanese';
	var KEY_DOI        = 'doi';
	var KEY_LINK_URL   = 'link-url';
	var KEY_LINK_TITLE = 'link-title';
	...
	btn.addEventListener('click', function () {
		...
		disableFilterButton();
		loadFiles(urls, keyToVals, lang, SEL_RESULT_HOLDER, enableFilterButton);
	});
...
console.log(items);
	...
	function processSheet(sheet, fkeyToVals, lang, retItems) {
		...
		for (var y = y0 + 1; y < y1; y += 1) {  // skip header
			...
			for (var x = x0; x < x1; x += 1) {
				...
				var key = colToKey[x];
				if (key === KEY_ENGLISH || key === KEY_JAPANESE) {
				...
				} else if (key === KEY_LINK_URL || key === KEY_LINK_TITLE) {
					if (cell && cell.w && cell.w.length > 0) {
						item[key] = cell.w;
						// console.log(key, cell.w);
					}
				} else if (KEY_ORDER_ROOT.indexOf(key) !== -1) {
					if (cell && cell.w && cell.w.length > 0) {
						var vals = cell.w.split(/\s*,\s*/);
						item[key] = vals.slice(0);  // make a clone

						for (var i = 0; i < vals.length; i += 1) {
							var sub = KEY_ANCESTOR[vals[i]];
							if (!sub) continue;
							for (var j = 0; j < sub.length; j += 1) item[key].push(sub[j]);
						}
						item[key] = item[key].map(function (x) {return x.trim();});
					}
				}
			}
			if (makeBody(item, lang) && isMatch(item, fkeyToVals)) {
				sortItemVals(item);
	...
	function compareItem(a, b) {
		for (var i = OPT_FIRST_KEY_OMITTED ? 1 : 0, I = KEY_ORDER_ROOT.length; i < I; i += 1) {
			var key = KEY_ORDER_ROOT[i];
			var order = KEY_ORDER[key];
			var avs = a[key], bvs = b[key];

			// var av = avs ? avs[avs.length - 1] : '';
			// var bv = bvs ? bvs[bvs.length - 1] : '';
			// if (av === '' && bv === '') continue;
            //
			// var ai = order[av] !== undefined ? order[av] : -1;
			// var bi = order[bv] !== undefined ? order[bv] : -1;
